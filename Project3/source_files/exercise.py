...
def runge_kutta(f, Jf, y0, t, a, B, c):
  ...
  """
    ...
    n = len(y0)  # dimension of the ODE system
    s = len(a)  # number of stages
    N = len(t)  # number of time pts
    ...
    for m in range(N - 1):
        ...
        for i in range(s):
            # known part of the stage argument (contributions from earlier stages)
            Y = np.copy(ym)
            for j in range(i):
                Y = Y + h * B[i, j] * k[j]
            ...
            if explicit or B[i, i] == 0:
                # explicit stage: no unknown k[i] on the right-hand side
                k[i] = f(ti, Y)
            else:
                # implicit stage: solve k[i] = f(ti, Y + h*B[i,i]*k[i]) by Newton's solver method
                ki = f(ti, Y)  # initial guess
                for it in range(100):
                    Yi = Y + h * B[i, i] * ki
                    g = ki - f(ti, Yi)
                    Jg = np.eye(n) - h * B[i, i] * Jf(ti, Yi)
                    delta = np.linalg.solve(Jg, -g)
                    ki_new = ki + delta
                    if np.linalg.norm(ki_new - ki) <= 1e-8:
                        ki = ki_new
                        break
                    ki = ki_new
                k[i] = ki
...
def k_step(f, Jf, y0, y1, t, a, b):
  ...
  """
    ...
    n = len(y0)
    N = len(t)
    k = len(a) - 1  # number of steps

    h = t[1] - t[0]  # uniform step size assumed for simplicyt here

    y = np.zeros((N, n))
    y[0] = y0

    # fill in the missing starting values y[1],...,y[k-1]
    if k > 1:
        if len(y1) == 0:
            sol = odeint(f, y0, t[0:k], tfirst=True)  # as said in the instruction sheet
            y[1:k] = sol[1:k]
        else:
            y1 = np.atleast_2d(y1)
            y[1:k] = y1

    explicit = (Jf == [])  # evalues true if Jf is empty bookean value fxn

    for m in range(N - k):
        # known part coming from y[m],...,y[m+k-1]
        rhs = np.zeros(n)
        for i in range(k):
            rhs = rhs - a[i] * y[m + i] + h * b[i] * f(t[m + i], y[m + i])

        tk = t[m + k]

        if explicit or b[k] == 0:
            # explicit: a[k]*y_new = rhs
            y[m + k] = rhs / a[k]
        else:
            # implicit: solve a[k]*y_new - h*b[k]*f(tk,y_new) = rhs by Newton's method
            y_new = rhs / a[k]  # initial guess
            for it in range(100):
                g = a[k] * y_new - h * b[k] * f(tk, y_new) - rhs
                Jg = a[k] * np.eye(n) - h * b[k] * Jf(tk, y_new)
                delta = np.linalg.solve(Jg, -g)
                y_new_next = y_new + delta
                if np.linalg.norm(y_new_next - y_new) <= 1e-8:
                    y_new = y_new_next
                    break
                y_new = y_new_next
            y[m + k] = y_new
