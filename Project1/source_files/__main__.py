...
def zplot(t,x,title,fname):
  ...
  path = ""                        # local user path for storing plots
  fs = 15 # FontSize
  Tend = t[len(t)-1]; 
  # plt.figure("/i,ages")
  plt.figure()
  ...
  # if(any(x[:,0]<1e-9)):
  #   print(f"Humans negative:")
  #   print(f"Humans: {x[:, 0]}")
  #
  # if (any(x[:, 1] < 1e-9)):
  #   print(f"Zombines negative:")
  #   print(f"Zombies: {x[:,1]}")
  # if (any(x[:, 2] < 1e-9)):
  #   print(f"Removed negative:")
  #   print(f"Removed: {x[:,2]}")


  return

p = 0.25 # uncertainty of parameters alpha and beta in percent
